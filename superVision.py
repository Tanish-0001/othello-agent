...
for epoch in range(4):
    ...
    for i, (states, actions, winners) in enumerate(trainLoader):
        ...
        optimizer.zero_grad()
        x = F.relu(resnet.gn_in(resnet.conv_in(states)))
        x = resnet.res_blocks(x)
        x = resnet.flatten(x)

        value = resnet.value(x)
        advantage = resnet.advantage(x)

        valueLoss = criterionValue(value, winners)
        
        winMask = (winners > 0).flatten()
        if winMask.any():
            advantageLoss = criterionAdvantage(advantage[winMask], actions[winMask])
        else:
            advantageLoss = 0.0

        totalLoss = valueLoss  + advantageLoss
        totalLoss.backward()
        optimizer.step()
        if i % 50 == 0:
